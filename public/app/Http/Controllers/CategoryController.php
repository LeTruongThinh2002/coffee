...
class CategoryController extends Controller
{
    ...
    function cdelete(int $id){
        ...
        if(isset($response->json()['old']))
            $arr=$response->json()['old'];
        if(isset($arr)){
    ...
    function edit(Request $req,int $id ){
        if($req->isMethod('post')){
            $data=$req->validate(['CategoryName'=>'required','Description'=>'required']);
            if($req->hasFile('ImageUrl')) {
                $name = Str::random(28).'.jpg';
                $data['ImageUrl']=$name;
            }else{
            ...
            if(isset($arr)){
                if($arr['test']){
                    return response()->json(['success' => true, 'CategoryName'=>$data['CategoryName'],'Description'=>$data['Description'],'ImageUrl'=>$arr['img']]);
                }
                else{
                    $req->file('ImageUrl')->move(public_path('/images/product/'), $name);
                    $text=$arr['img'];
            ...
            return response()->json(['success' => false,'khong tim thay']);  
        }   
        return response()->json(['success' => false]);
    }
}