    ...
    <div class="col-9">
        <div class="card">
            <div class="card-header bg-light">
                <div class="card-title text-dark fa-2x">
                    Edit product {{$pd['ProductName']}}
                </div>
            </div>
            <div class="card-body">
                        <div class="d-flex row ms-3">
                            <form id="submit-form" action="/product/doEdit/{{$pd['ProductId']}}" method="POST">
                                ...
                                <select class="form-select-sm bg-dark text-light rounded-pill" aria-label="Default select example" name="CategoryId">
                                    @foreach($cat as $v)
                                ...
                                @endforeach
                                <br>
                                <a class="btn btn-danger" href="/product"><i class="fas fa-times"></i></a>
                                <button id="btnNavbarSearch" type="submit" class="btn btn-warning"><i class="fas fa-check"></i></button>
                            </form>
                        </div>
                </div>
        </div>
        ...
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            success: function(data)
            {
                if(data.success){
                    ...
                    showToast('Không thể chỉnh sửa ','product do '.data.er);
            },
